...
def dab_threshold(image: np.ndarray, threshold: int, rescale: bool = False):
    ...
    if rescale:
        temp = rescale_intensity(temp, (temp.min(), temp.max()), (0, 255)).astype(np.uint8)
...
def closing(binary: np.ndarray, kernel_size: int):
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size)) if kernel_size > 0 else None
    temp = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel) if kernel is not None else binary
    return temp


def find_plaques(binary, minsize, return_masks, use_area=False):
    min_area = (minsize/2)**2 * np.pi
    num_components, labels, stats, centroids = cv2.connectedComponentsWithStats(binary, 4)
    result = []
