...
def get_grey_matter(row_pixel, column_pixel, height, width, grey_matter, downscale_factor):
...
def get_bordering_detections(bbs: list, iou_threshold=0):
    ...
    for i in range(n):
        for j in range(i+1, n):
            x_overlap = min(bbs[i].x + bbs[i].w, bbs[j].x + bbs[j].w) - max(bbs[i].x, bbs[j].x)
            y_overlap = min(bbs[i].y + bbs[i].h, bbs[j].y + bbs[j].h) - max(bbs[i].y, bbs[j].y)
            if x_overlap > 0 and y_overlap == 0:
                x_range = max(bbs[i].x + bbs[i].w, bbs[j].x + bbs[j].w) - min(bbs[i].x, bbs[j].x)
                iou = x_overlap / x_range
            elif y_overlap > 0 and x_overlap == 0:
                y_range = max(bbs[i].y + bbs[i].w, bbs[j].y + bbs[j].w) - min(bbs[i].y, bbs[j].y)
...
def get_bordering_detections_from_plaque_mask(pms: list, iou_threshold=0):
    ...
    for i in range(n):
        for j in range(i+1, n):
            x_overlap = min(pms[i].bb.x + pms[i].bb.w, pms[j].bb.x + pms[j].bb.w) - max(pms[i].bb.x, pms[j].bb.x)
            y_overlap = min(pms[i].bb.y + pms[i].bb.h, pms[j].bb.y + pms[j].bb.h) - max(pms[i].bb.y, pms[j].bb.y)
            if x_overlap > 0 and y_overlap == 0:
                x_range = max(pms[i].bb.x + pms[i].bb.w, pms[j].bb.x + pms[j].bb.w) - min(pms[i].bb.x, pms[j].bb.x)
                iou = x_overlap / x_range
            elif y_overlap > 0 and x_overlap == 0:
                y_range = max(pms[i].bb.y + pms[i].bb.w, pms[j].bb.y + pms[j].bb.w) - min(pms[i].bb.y, pms[j].bb.y)
                iou = y_overlap / y_range
            else:
                iou = 0
            if iou > iou_threshold:
...
def merge_plaque_masks(plaques: h5py.Group, ccs: list):
    ...
    for cc in ccs:
        ...
        merged_bb = p.BoundingBox(min_x, min_y, max_x - min_x, max_y - min_y)
        merged_mask = p.PlaqueMask(np.zeros((max_y - min_y, max_x - min_x), bool), merged_bb)
        for pm in pms:
            x = pm.bb.x - min_x
            y = pm.bb.y - min_y
