...
def maximum_inscribed_circle(mask: np.ndarray):
    ...
    padded = cv2.copyMakeBorder(mask.astype(np.uint8), 1, 1, 1, 1, cv2.BORDER_CONSTANT, 0)
...
def minimum_circumscribed_circle(mask: np.ndarray):
    ...
    contours, _ = cv2.findContours(mask.astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[-2:]
