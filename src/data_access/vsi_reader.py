...
class VsiReader(data.Dataset, IterMixIn):
    ...
    def get_full(self):
        if not self.fully_loaded:
            ...
            temp_patch_it = Patchifyer(
                self.shape[0], self.shape[1], self.SIZE_FULL_LOAD, self.SIZE_FULL_LOAD
            )
            for i in range(len(temp_patch_it)):
                r, c = temp_patch_it[i]
                patch = self.image_reader.rdr.openBytesXYWH(
                    0, c, r, self.SIZE_FULL_LOAD, self.SIZE_FULL_LOAD
                )
                patch = patch.reshape(
                    (self.SIZE_FULL_LOAD, self.SIZE_FULL_LOAD, 3)
                ).astype(self.dtype)
                self.image[r : r + self.SIZE_FULL_LOAD, c : c + self.SIZE_FULL_LOAD] = (
                    patch
                )
