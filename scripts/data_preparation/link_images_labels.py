...
import cv2 as cv
from split_data import shuffle_data, list_files
...
def link_images_labels(save_dir, linked_file_path):
    images_downsampled_dir = os.path.join(save_dir, 'processed', 'images')
    labels_downsampled_dir = os.path.join(save_dir, 'processed', 'labels')
    ...
    try:
        ...
        for label_file_path in label_files:
            ...
            images_downsampled_file_path = os.path.join(images_downsampled_dir, f'{label_name}.hdf5')
            labels_downsampled_file_path = os.path.join(labels_downsampled_dir, f'{label_name}.hdf5')
            
            if not os.path.exists(images_downsampled_file_path):
                raise FileNotFoundError(f"A matching downsampled image file not found: '{images_downsampled_file_path}'")
            ...
            label_group['image_file'] = h5py.ExternalLink(images_downsampled_file_path, '/')
            label_group['label_file'] = h5py.ExternalLink(labels_downsampled_file_path, '/')
...
if __name__ == "__main__":
    ...
    linked_file_path = os.path.join(args.save_dir, 'processed', 'linked_images_labels.hdf5')
