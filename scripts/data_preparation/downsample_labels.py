...
def parse_arguments():
    ...
    parser.add_argument('-ld', '--labels_dir', default=os.path.join('dataset', 'original', 'grey matter labels'), type=str)
    ...
    print(
        ...
        '\n\labels directory:', args.labels_dir
    )
...
def save_downsampled_labels(args):
    ...
    images_downsampled_dir = os.path.join(args.save_dir, 'processed', images_target_dir)
    labels_downsampled_dir = os.path.join(args.save_dir, 'processed', labels_target_dir)
    ...
    for i, label_file_path in enumerate(label_files):
        ...
        if replace_created or not os.path.exists(labels_downsampled_file_path):
            ...
            if not os.path.exists(images_downsampled_file_path):
                raise FileNotFoundError(f"A matching downsampled image file was not found: '{images_downsampled_file_path}'")
