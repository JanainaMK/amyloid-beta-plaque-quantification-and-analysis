...
def shuffle_data(file: h5py.File, train_size = 0.7, validation_size=0.1, random_state=42):
    print('\nassigning shuffled data to train, validation, and test set')
    
    key_list = [key for key in file['init'].keys()]
    random.shuffle(key_list)
    # Split the data into train, test, and validation sets
    train_names, temp_names = train_test_split(key_list, train_size=train_size, random_state=random_state)
    test_names, val_names = train_test_split(temp_names, test_size=validation_size, random_state=random_state)
