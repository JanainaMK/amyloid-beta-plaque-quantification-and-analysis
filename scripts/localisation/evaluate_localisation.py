...
for key in file:
    ...
    thresh, t = (
        pf.dab_threshold_otsu(image, dab_threshold, rescale)
        if use_otsu
        else pf.dab_threshold(image, dab_threshold, rescale)
    )
    ...
    if use_mask:
        confusion_matrix = ev.evaluate_masks(
            predictions, ground_truth, positive_criterion, strict
        )
    else:
        confusion_matrix = ev.evaluate_bounding_boxes(
            predictions, ground_truth, positive_criterion, strict
        )
