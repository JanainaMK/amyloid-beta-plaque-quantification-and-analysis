...
def main():
    ...
    image_names = file_names[start:stop]
    ...
    try:
        ...
        for i, image_name in enumerate(image_names):
            ...
            print('slide', i, ':', image_name)
        
            with h5py.File(f'{target_folder}/{image_name}.hdf5', 'a') as result_file:
                ...
                grey_matter = segment_gray_matter(segmentation_setting, model, vsi_reader, result_file, image_name)
                ...
                print('image shape:', vsi_reader.shape)
                print('patchifier shape:', vsi_reader.patch_it.shape)
                plaque_masks = locate_plaques(vsi_reader, grey_matter, threshold, kernel_size, minsize, start, downscale_factor)
...
def parse_arguments():
    parser = argparse.ArgumentParser(prog='Segmentation and Localisation pipeline',
    ...
    parser.add_argument('-ss', '--segmentation_setting', choices=['no', 'load', 'create'], default='create',
...
def segment_gray_matter(segmentation_setting, model, vsi_reader, result_file, image_name):
...
def locate_plaques(vsi_reader, grey_matter, threshold, kernel_size, minsize, start, downscale_factor):
    pms = []
    for i in range(len(vsi_reader)):
        ...
        print('analyzing patch', f'{i}/{len(vsi_reader)}, loc:({r}, {c}), shape: ({h}, {w})')
