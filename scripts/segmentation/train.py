...
if __name__ == "__main__":
    ...
    data_file = './dataset/processed/linked_images_labels.hdf5'
    
    # opens HDF5 file in read-only mode
    with h5py.File(data_file, 'r') as file:
        train_reader = DatasetReader(file['train'], patch_size, stride, downsample_lvl, label_type, dtype=np.float64)
        # batches and creates an iterable over the train dataset
        train_loader = DataLoader(train_reader, batch_size, shuffle=True, num_workers=num_workers)
        ...
        # batches and creates an iterable over the validation dataset
        validation_loader = DataLoader(validation_reader, batch_size, shuffle=True, num_workers=num_workers)
    ...
    # Defines binary cross-entropy loss function
    criterion = torch.nn.BCELoss()
    print('model and optimizer ready',  time.time() - start_time, flush=True)
    
    # Creates directories for saving losses and models
    os.makedirs(f'./loss/{run_name}', exist_ok=True)
    os.makedirs(f'./models/{run_name}', exist_ok=True)
    # Arrays are initialized to store training and validation loss
    ...
    for e in range(epochs):
        ...
        print('epoch', e)
        avg_train_loss, info = train_epoch(model, train_loader, optim, criterion, info_it=None, print_times=args.timed)
        ...
        torch.save(model.state_dict(), f'./models/{model_type}/{run_name}-e{e}.pt')
